...
- 掲示板マスタの座標位置にピンを表示。ピンはステータス（`未着手`=グレー / `貼付済`=緑 /
  `トラブル`=赤）によって色分けされる。
- ピンをタップするとポップアップが開き、ステータスと担当者の変更が可能（更新は
  `POST /api/board/update`。変更内容は`poster_activity_log`に自動記録される）。
- ヘッダの担当者フィルタで、選択した担当者の掲示板のみに絞り込める。
- ヘッダのステータスフィルタ（3つのトグルボタン）で、表示するステータスを選べる
  （複数選択可。既定は全表示）。
...
- **対象地域は大和市のみ**（2026-08時点、動作確認・初回デプロイ未実施）。他自治体を追加する
  場合は「掲示板マスタCSVの変換」の手順に沿って専用の変換スクリプトを用意する。
...
- **本番未デプロイ**: `npm run new-region`の実際のCloudflare操作（D1作成・Secrets設定・deploy）
  を伴う箇所は実機（Cloudflare認証情報がある環境）で未検証。初めて実行する際は各ステップの
  出力（特に`wrangler d1 create`の`database_id`抽出）を確認しながら進めること。
